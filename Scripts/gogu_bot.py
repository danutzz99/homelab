...
async def banned_ips(ctx):
    """List IPs banned by HoneyAuth via Cloudflare."""
    ...
    try:
        ...
        if not data.get("success"):
            await ctx.send("❌ Failed to fetch banned IPs from Cloudflare.")
            return
        ...
        lines = ["🛡️ **HoneyAuth Banned IPs**", "```"]
        for ban in bans[:20]:  # Limit to 20
